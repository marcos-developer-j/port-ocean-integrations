#!/usr/bin/env bash
# Construye las imágenes Docker de las 5 integraciones custom y las carga en el cluster kind.
...
#   ./build-and-load.sh port-ocean confluence      # solo confluence
set -euo pipefail
...
if [ ${#INTEGRATIONS[@]} -eq 0 ]; then
  INTEGRATIONS=(jfrog orca veracode github-projects confluence)
fi
